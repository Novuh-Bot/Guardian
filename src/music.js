...
const commands = {
	'play': (msg) => {
		if (queue[msg.guild.id] === undefined) return msg.channel.send(`Add some songs to the queue first with ${settings.prefix}add`);
		...
		(function play(song) {
			...
			msg.channel.send(`<:music:${settings.music}> Playing: **${song.title}** as requested by: **${song.requester}**`);
	...
	'add': (msg) => {
		...
		if (url == '' || url === undefined) return msg.channel.send(`You must add a YouTube video url, or id after ${settings.prefix}add`);
		yt.getInfo(url, (err, info) => {
			if(err) return msg.channel.send('Invalid YouTube Link: ' + err);
			if (!queue.hasOwnProperty(msg.guild.id)) queue[msg.guild.id] = {}, queue[msg.guild.id].playing = false, queue[msg.guild.id].songs = [];
			queue[msg.guild.id].songs.push({url: url, title: info.title, requester: msg.author.username});
			msg.channel.send(`<:music:${settings.music}> Added **${info.title}** to the queue`);
		});
	},
	'queue': (msg) => {
		if (queue[msg.guild.id] === undefined) return msg.channel.send(`Add some songs to the queue first with ${settings.prefix}add`);
...
client.on('message', msg => {
	...
	if (commands.hasOwnProperty(msg.content.toLowerCase().slice(settings.prefix.length).split(' ')[0])) commands[msg.content.toLowerCase().slice(settings.prefix.length).split(' ')[0]](msg);
});
